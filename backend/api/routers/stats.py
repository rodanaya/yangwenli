...
def get_fast_dashboard(response: Response):
    ...
    with get_db() as conn:
        ...
        # P1: Multivariate anomaly count — use precomputed value, fallback only if missing
        multivariate_anomaly_count = stats.get('multivariate_anomaly_count')
        if multivariate_anomaly_count is None:
            try:
                row = cursor.execute(
        ...
        # P1: Election year risk comparison — use precomputed values, fallback only if missing
        ...
        if election_year_avg_risk is None:
            try:
        ...
        # P1: New vendor risk count — use precomputed value, fallback only if missing
        new_vendor_risk_count = stats.get('new_vendor_risk_count')
        if new_vendor_risk_count is None:
            try:
        ...
        # P1: Data quality grade distribution — use precomputed values, fallback only if missing
        grade_a_pct = stats.get('grade_a_pct')
        grade_b_pct = stats.get('grade_b_pct')
        if grade_a_pct is None:
            try:
        ...
        # P1: Sexenio comparison — use precomputed value, fallback only if missing
        sexenio_comparison = stats.get('sexenio_comparison')
        if sexenio_comparison is None:
            try:
