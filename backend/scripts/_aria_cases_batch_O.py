"""
ARIA Cases Batch O: 4 T3 vendors from ARIA queue investigation (Mar 20 2026).

Investigated 6 vendors, added 4, skipped 2:
  ADD: 15615 (INTERNACIONAL DE CALZADO TEN PAC) — PEMEX footwear SB+DA capture
  ADD: 18948 (AVANZADA TECNICA CONSTRUCTIVA) — ISSSTE/AICM construction SB capture
  ADD: 3837 (TELETEC DE MEXICO) — INBA + multi-state telecom SB manipulation
  ADD: 139508 (P&C LIMPIEZA) — ISSSTE/AICM cleaning DA capture
  SKIP: 2970 (CALZADO VAN VIEN) — diversified footwear supplier, no institutional capture
  SKIP: 6607 (INSELEC) — diversified electrical supplier, moderate SB but no concentration
...
DATA = Path(__file__).resolve().parent / "_batch_O_data.json"

# Vendors investigated but skipped (update aria_queue only)
SKIPPED_VENDORS = [2970, 6607]
...
def run():
    ...
    # Update aria_queue for all 4 added vendors
    ...
    # Update aria_queue for 2 skipped vendors (mark reviewed but not GT)
    for vid in SKIPPED_VENDORS:
        conn.execute(
            "UPDATE aria_queue SET review_status='reviewed_clean' WHERE vendor_id=?",
            (vid,)
        )
    ...
    last = next_id + n - 1
    print()
    print("Batch O complete:")
    print(f"  Cases added: {n} (IDs {next_id}-{last})")
    print(f"  Contracts tagged: {tagged_total}")
    print(f"  Skipped vendors (reviewed_clean): {SKIPPED_VENDORS}")
