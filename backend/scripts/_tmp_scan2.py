...
FROM aria_queue WHERE ips_tier=3 AND review_status='pending' AND in_ground_truth=0 AND fp_structural_monopoly=0 AND fp_data_error=0 AND fp_patent_exception=0 AND total_value_mxn > 200000000 ORDER BY ips_final DESC LIMIT 25 OFFSET 25''').fetchall()
for r in rows:
