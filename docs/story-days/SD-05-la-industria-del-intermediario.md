...
Built by `story-day05` (report `_parallax_shots/story-days/sd05-report.md`); judged by Fable on `sd05/after/` (f2/f3 at 1440 EN, f4 at 390 ES). Headline kept ("Follow the Middleman" / "Sigan al Intermediario" — nothing false in it); subheadline rewritten; ARHNOS (false positive) removed everywhere; new lead MANEJO INTEGRAL EN CONSULTA EMPRESARIAL (136648: 6.31B in 2 contracts at COMESA 2014, ticket 3.15B = 504× the energy average, T1, GT, disappeared); the 36 largest P3 vendors at the 12 top buyers: 6 cleared hold 51.5% of the money → "the biggest names are not the best leads"; COFECE claim dropped; 2,691 of 2,972 never opened. Deviations accepted: HTML channel ledger instead of `MoneySankeyChart` (dead code, clips labels, breaks rules 1/3); `/aria/queue` has no `sort` — value rankings come from the Day 4 aggregate's inline vendor rows (the only source carrying `review_status`); F4 is a disposition ledger rather than a roster; `AriaQueueItem.review_status` type widened to the nine real statuses; Spanish log axis re-labelled in MDP. Gates: tsc 0 · build OK · lint:tokens PASS · probe 111/0 · census 0 narrow captions, 0 structural clips (EN + ES, 4 widths).

Deployed 2026-09-19 ≈04:50 CET via `deploy-safe.sh` (VPS HEAD `4f7e05f0`). Commits `e76ed14b` · docs · BUILD_ID `4f7e05f0` (`2026-09-19-story-d5-intermediario`). Entry `index-CjEQ0Jbo.js` → `index-Fn3sE0Bv.js`; BUILD_ID string served; health OK.
