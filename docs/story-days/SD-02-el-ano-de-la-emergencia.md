...
Deployed 2026-09-18 19:04Z via `deploy-safe.sh` (`[deploy] done`, VPS HEAD `ffc60c4a`). Commits `ebd9897c` (figures + fact rewrite) · `1f13277e` (judge: headline/card alignment, zero-clip fixes) · docs · BUILD_ID `ffc60c4a` (`2026-09-18-story-d2-emergencia`). Entry bundle `index-Cr91clQH.js` → `index-vpIr-RHr.js`; BUILD_ID string 1 hit in the served entry chunk; health `db_connected: true`, 3,058,286 contracts. Judge verified independently: `/analysis/year-over-year` 2019 77.8 → 2020 78.1 → 2021–24 80.0 / 79.1 / 82.2 / 79.4; HEMOSER 2020 = 54 awards, 4.48B, 97.0% IMSS, 46 direct awards carrying 4.6% of value, top two awards LICITACIÓN PÚBLICA Aug 2020; ARIA P7 / T2 / confirmed_corrupt / P2 = 0.0.
