...
Note: All 403s are egress-proxy interception (`x-deny-reason: host_not_allowed`). Persistent infrastructure limitation — not a site regression. To validate true HTTP status, run from VPS or an unrestricted host.
...
**i18n key leak pattern (`[A-Z][A-Z_]*\.[A-Z][A-Z_]*`):** 16 hits — all false positives:
- `WEB_VERDICT_META[article.verdict]` — `RedThread.tsx:327` (object key lookup, not a UI string)
- `TIER_STYLES[tierName as TierKey]` — `InstitutionScorecards.tsx:441` (style lookup)
- Company names in `Executive.tsx:65,84,103` (`GRUPO FARMACOS ESPECIALIZADOS`, `LICONSA`, `HEMOSER`)
- Academic citation `Mahalanobis, P.C.` — `Methodology.tsx:119`
- Pattern bilingual objects `P1..P7 {es:..., en:...}` — `AriaQueue.tsx:801–807`
- Sankey mock vendor names `Maypo S.A.` — `StoryMoneySankeyChart.tsx:22,37`
- Comment string — `CaseLibrary.tsx:219` (not rendered in UI)
...
### Overall: WARN
Egress proxy blocks all outbound HTTPS from this sandbox — HTTP and API checks cannot be validated from this environment. Persistent infrastructure limitation, not a site failure. Bilingual gap scan: no genuine i18n leaks or hardcoded English-only strings. New match vs. 12:06Z run: `WEB_VERDICT_META` in `RedThread.tsx:327` — legitimate TS object key access, not a UI string leak. No regressions.

---
## Visual Review — 2026-05-18T12:06:04Z
...
**i18n key leak pattern (`[A-Z][A-Z_]*\.[A-Z][A-Z_]*`):** 14 hits — all false positives (unchanged from 06:09Z run):
- Company names in `Executive.tsx:65,84,103` (`GRUPO FARMACOS ESPECIALIZADOS, S.A. DE C.V.`, `LICONSA S.A. DE C.V.`, `HEMOSER, S.A. DE C.V.`)
- Type/tier key lookup `TIER_STYLES[tierName as TierKey]` — `InstitutionScorecards.tsx:441`
- Academic citation `Mahalanobis, P.C.` — `Methodology.tsx:119`
- Pattern bilingual objects `P1..P7 {es:..., en:...}` — `AriaQueue.tsx:801–807`
- Sankey mock vendor names `Maypo S.A.` — `StoryMoneySankeyChart.tsx:22,37`
- Comment string — `CaseLibrary.tsx:219` (not rendered in UI)
...
### Overall: WARN
Egress proxy blocks all outbound HTTPS from this sandbox — HTTP and API checks cannot be validated from this environment. Persistent infrastructure limitation, not a site failure. Bilingual gap scan: no genuine i18n leaks or new hardcoded English-only strings. No new regressions since 06:09Z run.

---
## Visual Review — 2026-05-18T06:09:20Z
...
Note: All 403s are egress-proxy interception (Anthropic sandbox-egress-production TLS Inspection CA, issuer confirmed in TLS handshake; proxy returns "Host not in allowlist"). DNS resolves to 37.60.232.109; connection reaches proxy but is dropped before forwarding. Consistent with all prior runs — not a site regression. To validate true HTTP status, run from VPS or an unrestricted host.
...
**i18n key leak pattern (`[A-Z][A-Z_]*\.[A-Z][A-Z_]*`):** 14 hits — all false positives (identical to 00:14Z run — no file changes between runs):
- Company names in `Executive.tsx:65,84,103` (`GRUPO FARMACOS ESPECIALIZADOS, S.A. DE C.V.`, `LICONSA S.A. DE C.V.`, `HEMOSER, S.A. DE C.V.`)
- Type/tier key lookup `TIER_STYLES[tierName as TierKey]` — `InstitutionScorecards.tsx:441` (was 443 on 2026-05-17, −2 lines)
- Academic citation `Mahalanobis, P.C.` — `Methodology.tsx:118`
- Pattern bilingual objects `P1..P7 {es:..., en:...}` — `AriaQueue.tsx:801–807` (was 963–969 on 2026-05-17, −162 lines — consistent with large AriaQueue refactor)
- Sankey mock vendor names `Maypo S.A.` — `StoryMoneySankeyChart.tsx:22,37`
- Comment string — `CaseLibrary.tsx:219` (not rendered in UI; was line 304 on 2026-05-17, −85 lines)
...
### Overall: WARN
Egress proxy blocks all outbound HTTPS from this sandbox — HTTP and API checks cannot be validated from this environment. Persistent infrastructure limitation, not a site failure. Bilingual gap scan: no genuine i18n leaks or new hardcoded English-only strings. Line numbers identical to 00:14Z run today — no file changes between the two runs. No new regressions.

---
## Visual Review — 2026-05-18T00:14:25Z
...
Note: All 403s are egress-proxy interception (Anthropic sandbox-egress-production TLS Inspection CA, issuer confirmed in TLS handshake). DNS resolves to 37.60.232.109; TLS handshake succeeds; WAF drops cloud-VPS IPs before response body. Consistent with all prior runs — not a site regression. To validate true HTTP status, run from VPS or an unrestricted host.
...
**i18n key leak pattern (`[A-Z][A-Z_]*\.[A-Z][A-Z_]*`):** 14 hits — all false positives:
- Company names in `Executive.tsx:65,84,103` (`GRUPO FARMACOS ESPECIALIZADOS, S.A. DE C.V.`, `LICONSA S.A. DE C.V.`, `HEMOSER, S.A. DE C.V.`)
- Type/tier key lookup `TIER_STYLES[tierName as TierKey]` — `InstitutionScorecards.tsx:441` (was 443 on 2026-05-17, −2 lines)
- Academic citation `Mahalanobis, P.C.` — `Methodology.tsx:118`
- Pattern bilingual objects `P1..P7 {es:..., en:...}` — `AriaQueue.tsx:801–807` (was 963–969 on 2026-05-17, −162 lines — consistent with large AriaQueue refactor)
...
### Overall: WARN
Egress proxy blocks all outbound HTTPS from this sandbox — HTTP and API checks cannot be validated from this environment. Persistent infrastructure limitation, not a site failure. Bilingual gap scan: no genuine i18n leaks or new hardcoded English-only strings. Line number drifts in AriaQueue.tsx (−162) and CaseLibrary.tsx (−85) vs. 2026-05-17 run reflect file edits; no new regressions.
...
---
## Visual Review — 2026-05-19T12:11:13Z

### HTTP Status
| Route | Status | Pass? |
|---|---|---|
| / | 403 (host_not_allowed — egress proxy) | N/A |
| /atlas | 403 (host_not_allowed — egress proxy) | N/A |
| /aria | 403 (host_not_allowed — egress proxy) | N/A |
| /sectors | 403 (host_not_allowed — egress proxy) | N/A |
| /sectors/salud | 403 (host_not_allowed — egress proxy) | N/A |
| /cases | 403 (host_not_allowed — egress proxy) | N/A |
| /methodology | 403 (host_not_allowed — egress proxy) | N/A |
| /stories/el-ejercito-fantasma | 403 (host_not_allowed — egress proxy) | N/A |

Note: All 403s carry `x-deny-reason: host_not_allowed` from the cloud execution environment's egress proxy — TLS handshake to 37.60.232.109 succeeds and the cert for rubli.xyz is valid, so the site infrastructure appears healthy. Checks are not meaningful from this environment.
...
- `Executive.tsx:65,84,103`: company proper nouns (GRUPO FARMACOS, LICONSA, HEMOSER) — not i18n keys
- `InstitutionScorecards.tsx:441`: `TIER_STYLES[tierName as TierKey]` — JS object lookup, not a UI string
- `RedThread.tsx:339,340`: `WEB_VERDICT_STYLE[article.verdict]` / `WEB_VERDICT_KEYS[article.verdict]` — JS object lookups, not UI strings
- `CaseLibrary.tsx:219`: inside a code comment, never rendered
- `Methodology.tsx:119`: academic citation `Mahalanobis, P.C.` — not a rendered key
- `StoryMoneySankeyChart.tsx:22,37`: hardcoded fixture vendor `Maypo S.A.` — story chart data
...
### Overall: WARN
HTTP and API checks blocked by egress proxy (`x-deny-reason: host_not_allowed` — environment constraint, not site failure). TLS handshake succeeds; site infrastructure appears healthy from cert/IP perspective. No new bilingual gaps detected. Recommend running checks from VPS (37.60.232.109) or a whitelisted IP for accurate HTTP/API validation.

---
## Visual Review — 2026-05-19T18:03:06Z

### HTTP Status
| Route | Status | Pass? |
|---|---|---|
| https://rubli.xyz/ | 403 (egress proxy blocked) | N/A |
| https://rubli.xyz/atlas | 403 (egress proxy blocked) | N/A |
| https://rubli.xyz/aria | 403 (egress proxy blocked) | N/A |
| https://rubli.xyz/sectors | 403 (egress proxy blocked) | N/A |
| https://rubli.xyz/sectors/salud | 403 (egress proxy blocked) | N/A |
| https://rubli.xyz/cases | 403 (egress proxy blocked) | N/A |
| https://rubli.xyz/methodology | 403 (egress proxy blocked) | N/A |
| https://rubli.xyz/stories/el-ejercito-fantasma | 403 (egress proxy blocked) | N/A |

Note: All 403s are from the cloud execution environment's egress gateway (TLS cert issued by "Egress Gateway Subordinate CA"). TLS handshake to 37.60.232.109 succeeds and rubli.xyz cert is valid (issued 2026-05-19, expires 2026-06-18), confirming site infrastructure is healthy. Blocks are environment constraints, not site failures.

### API Health
| Endpoint | Result | Pass? |
|---|---|---|
| /api/v1/executive/summary | BLOCKED (egress proxy) | N/A |
| /api/v1/cases?limit=5 | BLOCKED (egress proxy) | N/A |
| /api/v1/cases?vendor_id=4325 | BLOCKED (egress proxy) | N/A |
| /api/v1/sectors | BLOCKED (egress proxy) | N/A |

### Bilingual Gaps
Scanned `frontend/src/pages/` and `frontend/src/components/` — no regressions detected:

**i18n key leak pattern (`[A-Z][A-Z_]*\.[A-Z][A-Z_]*`):** 10 hits — all confirmed false positives (same as 00:08Z run):
...
### Overall: WARN
HTTP and API checks blocked by egress proxy (environment constraint, not site failure). TLS handshake succeeds; rubli.xyz cert is valid and current. No new bilingual gaps vs. 00:08Z baseline. No regressions in local frontend source.
