...
| 9 | Institution dossier + ranking | `/institutions` `/institutions/:id` | `InstitutionLeague`, `InstitutionDossier`, `ReportCard`, `components/institution/*`, `backend/api/routers/institutions.py` | GOLD/enhance-in-place · STEP 0 three unrouted institution pages + orphans removed (−4,116) · 8 changes: **the two dead panels** — `top-categories` 404'd on every dossier, prod included (filtered on a column `contracts` does not have) → precomputed `institution_category_stats` (46,193 rows, loaded on prod in-container) on the canonical `categories` taxonomy, `risk-waterfall` precompute-first (IMSS > 280 s → 0.02 s), console errors 3 → 0, «What they buy» prints category chips · league honesty (zeros print `0` + sentence, `n = 12` thin-sample marker, the trend arrow that never rendered) · rows are links (`tr[role=button]` 50 → 0, nested 100 → 0, focus 131 → 0, targets 52 → 0) · plate callouts as measured HTML labels with full names + keyboard path (sub-10px 2/6 → 0) · `TIER_STYLES.ink` AA twins (contrast 224 → 1 large) · 1,010 frame, one footer, the empty Act removed · register `li` measure 686 → 960 (the D5 backlog prediction), waterfall 8.5 → 11px, shared `RiskOverTimePanel` decorative, `VIH/SIDA` via `formatEntityName` · phones: ranking as cards (scrollers 3 → 0), pills wrap, full names · judge `16193e7f` (one zero wording, ES confidence bands, search row) · backend 893 passed · BUILD_ID `2026-09-23-parallax-d9-institutions` — `docs/parallax/DAY-09-institutions.md` · **9b (four-perspective Opus panel: reader 62 · architect 71 · a11y 70 · dataviz 64)**: tier-filter pills HTTP 500 on prod (`scorecards.py` zip misalignment) · percentile `or 0.5` swallowing a true 0.0 · P90 printed 0/1 · ES lede/aria «indicador de riesgo» for the integrity score · hero/strip on stale `institutions.total_*` (673,326 → 662,001) · headline population 476 → 445 = the plate's · masthead window 2002–2025 + «last contract YYYY» marks · seal «Procedure ·», boleta «C · Adequate», reading sentence generated from the three verdicts · category precompute reloaded on prod (IMSS Medications 145,131 → 173,351) + «6 of 71 categories» · register on one base, HHI years, 4 × «⚑ review» above 10B · plate on 20–100 with the note inside its band, zero ticks, callouts seated in exposure order · `TIER_STYLES.ink` ramp ΔE 2.1 → 15.7 · a11y: pressed states, 12/12 sector pills AA, unique names, live region, focus to the results heading, roving radios, drop cap readable · **Change 0 dry run only: External Flags pillar counted contracts not vendors — 146 grades would change (IMSS 49.0 C → 54.8 C+); rescore awaits the user** · judge `8aa546d5` · backend 907 passed · BUILD_ID `2026-09-24-parallax-d9b-institutions-panel` — `docs/parallax/DAY-09b-institutions-panel.md` | ✅ Sep 23 · 9b Sep 24 |
| 10 | Panel (Dashboard) | `/dashboard` | `Executive.tsx`, `components/dashboard/MacroArc`, `components/executive/*` | ⭐ GOLD/enhance-in-place · STEP 0 `LensVisualization` + the unused `executive` i18n namespace removed (−314) · 8 changes: **honesty** — byline `Updated May 2026` → `Data cut 2025·09·28`, **one calendar** (seismograph AMLO 2018→2019 / Sheinbaum 2024→2025 from `lib/administrations.ts`, green band gone), MacroArc floored at **2010** (it drew the 2001 stub and 0 % for 2002–2009 — Structure A has no procedure type; "Four administrations"), lead-time median **computed** (2.7 typed → 3.5), P6 `15,923` ×2 → live `pattern_counts` 15,939, the pesos caveat and the schematic histogram say what they are · every figure at **1:1 with HTML glyphs** (`useMeasuredWidth`; 116px gutters / 62–117px letterboxes → 0; sub-10px 102 → 8 @1440, 197 → 28 @390 — all in the shared `ObservatoryScatter`) · links not roles (`role=link` 21 → 0, raw `<a>`/nav-buttons → `<Link>`, focus 26 → 8 atlas-owned, targets 4 → 0) · section h2s (6 → 17, skip 1 → 0), lens as `aria-pressed` group in `?lens=` · inks (contrast 114 → 33 = PlateFrame 21 + atlas strip 3 + 7 shared-chip names + 2 large h1 spans; `RISK_INK_ON_PLATE` twins) · 1,010 frame centred, one colophon (2 → 1), 15px intros, `captionFull` ×7 (38 → 95 %) · phones (truncation 13 → 0, stacked Cascade rungs) · `ChapterTiles` + `CaptureLeaders` extracted (tiles no longer remount on hover) · judge `e36a2a75` (full era names at 390, unbreakable tile values, chip risk ink restored) · BUILD_ID `2026-09-24-parallax-d10-panel` — `docs/parallax/DAY-10-panel.md` · **10b (user review, Sep 24)**: zero `…` on `/categories`, `/categories/:id`, `/vendors/:id` (24 → 0 ellipses, 21 → 0 clipped), El Alzado names all 19 head columns (in-column or index badge + legend, seated callouts) + a 3px risk-tier cap, the ARIA memo re-set in the dossier voice (bilingual, RFC redacted), track-record titles in full, dead `/aria/:id` link → `/aria?q=` · `6c702ee9`, BUILD_ID `2026-09-24-parallax-d10b-polish` — `docs/parallax/DAY-10b-categories-vendor-polish.md` | ✅ Sep 24 · 10b Sep 24 |
| 11 | Categorías | `/categories` `/categories/:id` | `CategoriesIndex`, `CategoryDossier` | 4 sub-10px; El Alzado Jul 3 · re-audit after 10b: 6 honesty defects (drift chart 0.5% for 49%, ×2/×3 proof bars, two HR ladders, three risk scales, OECD-30% copy over an EU-10% line) | 🔨 Sep 24 |
...
From Day 9 (Institutions):
- Plate leader lines may pass behind another callout's paper (`SpectralRegister`) — add the seated label boxes as obstacles for the leaders, or route leaders around them.
- `ReportCard` (`?tab=reporte`) at 390: 3 leaves at 6.1/3.8px inside the shared `BenchmarkRow` (same item as Day 8's) — fix in the primitive.
- `risk-profile` is 16.6 s cold for IMSS (two walks of the institution's contracts; 1h cache) — an `institution_stats`-style precompute would make it instant.
- `useLeagueField` assembles the 476-row federal field in 5 requests (`per_page` cap 100) — a 500 cap on the scorecards list endpoint is one line.
- `SpectralRegister` `domainMax = 86` is a constant; the axis top should follow the data (max score 78 today).
- The dossier fires 9 queries on mount; the largest contracts (4.2 s) and the timeline could be deferred below the fold.
- `ScorecardWidgets.tsx` keeps `GradeBadge10` / `InstitutionScorecardCard` exports nobody imports since STEP 0 — trim when the vendor hero (its remaining importer) is touched on Day 13.
- `institution_category_stats` has no refresh path (same class as the Day 8 admin precompute) — run `scripts/precompute_institution_categories.py` after any rescore and reload on prod.

From Day 9b (Institutions panel):
- ~~DECISION (user): rescore `institution_scorecards`~~ → **APPLIED 2026-09-24** (user said yes at the start of Day 10): `compute_scorecards --institutions-only` on the local DB, 146 grade moves exactly as the dry run (C→C+ 61, C+→B 58, D→C 19, B→B+ 5, D→D- 1, C→D 1, C+→C 1), checksum (2537, 141069.6, 256641829.1); dumped to `_parallax_shots/day10/rescore/institution_scorecards.sql` (stale backup beside it), loaded on prod in-container + `wal_checkpoint(TRUNCATE)` → (0,0,0), fresh-exec checksum equal; prod `/scorecards/institutions/251` serves 54.8 C+ (was 49.0 C), federal D count 98 → 83. No deploy needed (data only).
- EN tier word "Adequate" for `Regular` (backend label "Preocupante") — rename is the user's call; the letter grade now prints beside it.
- Honor-roll recency floor (policy): 44 of the first 50 rows last contracted before 2018; UNAM #1 on 40 contracts from 2003.
...
From Day 10b (categories + vendor polish):
- Shared StatStrip sub-line sits 3px into its padding at 1024; `/aria` expanded row: readout runs into the memo band and factor labels are cut; the vendor ARIA link uses an ExternalLink icon for an internal link; pre-existing ESLint errors in `AriaQueue.tsx` (set-state-in-effect) and `lib/contract-audit.ts` (control-char regex).
