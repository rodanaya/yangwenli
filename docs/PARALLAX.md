...
- **DECISION (user):** rescore `institution_scorecards` with the fixed External Flags loader (`compute_scorecards.py`, distinct ARIA vendors) — dry run `_parallax_shots/day09b/rescore-dryrun.md`: 146 grade changes (C→C+ 61, C+→B 58, D→C 19, B→B+ 5, D→D- 1, C→D 1, C+→C 1); IMSS 49.0 C → 54.8 C+. Then dump + prod load + a `/institutions` before/after.
