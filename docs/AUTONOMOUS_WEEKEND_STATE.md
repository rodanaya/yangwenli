...
| 1 (continue) | 2026-05-09 06:55 | Ship Phase 3 URL state sync (commit `55f07b5`) — `/explore?s=salud&i=251&v=29277` now deep-links to focused vendor inside institution inside sector. New `useExploreUrlSync` hook, new `hydrate-from-url` + `pop-to-level` actions. Breadcrumbs use single-dispatch pop. |
| 2–N | 2026-05-09 → 2026-05-10 | **Massive single-day push (~52 commits).** All summarized in `docs/PLAN_36H_2026-05-09.md` cycle-log; highlights: Phase 7 (Spatial Map promoted to `/`), all 39 story charts bilingual, mobile briefing drawer, keyboard shortcuts, Methodology hero/tiers/limitations i18n, Categories 502 throttle, story-chip cross-link, recent-jumps in ⌘K, "Map" CTA on vendor profile, ErrorBoundary stale-chunk recovery, segalmex/odebrecht 404-slug filter, watchlist-alerts auth gate. |
