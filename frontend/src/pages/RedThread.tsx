  ...
  return (
    <section id="chapter-subject" className="min-h-screen flex flex-col justify-center py-24 px-8 max-w-4xl mx-auto">
      ...
      >
        {[
          ...
          <div key={s.label} className="bg-stone-900 border border-stone-800 rounded-xl p-5">
            <p className="editorial-label text-stone-500 mb-1">{s.label}</p>
      ...
      <motion.div
        ...
        className="mt-16 flex items-center gap-2 text-stone-600"
      >
  ...
  return (
    <section id="chapter-timeline" className="min-h-screen py-24 px-8 max-w-5xl mx-auto">
      ...
      <div ref={ref} className="relative">
        {/* Year axis */}
        <div className="flex justify-between text-xs text-stone-600 mb-4 px-1">
        ...
        {/* Dot field */}
        <div className="relative h-48 bg-stone-950 border border-stone-800 rounded-xl overflow-hidden px-4 py-4">
          {dots.map((dot, idx) => {
        ...
        <div className="flex items-center gap-6 mt-4">
          {Object.entries(RISK_DOT_COLORS).map(([level, color]) => (
            <div key={level} className="flex items-center gap-2">
              ...
              <span className="text-xs text-stone-500 capitalize">{level}</span>
            </div>
          ))}
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3 mt-8">
        {years.slice(-12).map((year) => {
          ...
          return (
            <div key={year} className="bg-stone-900 border border-stone-800 rounded-lg p-3">
    ...
    <section id="chapter-pattern" className="min-h-screen py-24 px-8 max-w-4xl mx-auto">
      ...
      {meta && ariaPattern && (
        ...
        >
          ...
          <p className="text-stone-300 text-sm leading-relaxed">{meta.description}</p>
        </motion.div>
      )}
        ...
        {sorted.map((f, idx) => {
          ...
          return (
            <motion.div
              ...
              className="relative rounded-lg border border-stone-800 overflow-hidden"
            ...
            >
              ...
              <div className="relative flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-sm text-stone-200">{f.label_en}</span>
                  {f.z_score !== 0 && (
      ...
      {waterfall.length === 0 && (
        <div className="text-stone-600 text-sm italic">Feature attribution data not available for this vendor.</div>
      )}
    ...
    <section id="chapter-network" className="min-h-screen py-24 px-8 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        <div className="bg-stone-900 border border-stone-800 rounded-xl p-6">
          <p className="editorial-label text-stone-500 mb-2">Institutions Served</p>
        ...
        <div className="bg-stone-900 border border-stone-800 rounded-xl p-6">
          <p className="editorial-label text-stone-500 mb-2">Sectors Active</p>
      ...
      <Link
        ...
        className="group flex items-center gap-4 bg-stone-900 hover:bg-stone-800 border border-stone-800 hover:border-stone-600 rounded-xl p-6 transition-all mb-6"
      >
        <div className="w-12 h-12 rounded-full bg-stone-800 group-hover:bg-red-950 flex items-center justify-center transition-colors flex-shrink-0">
          <GitBranch className="w-5 h-5 text-stone-400 group-hover:text-[#dc2626] transition-colors" />
        </div>
        ...
        <ExternalLink className="w-4 h-4 text-stone-600 group-hover:text-stone-400 transition-colors flex-shrink-0" />
      </Link>

      {/* CTA to vendor profile network tab */}
      <Link
        to={`/vendors/${vendorId}?tab=network`}
        className="group flex items-center gap-4 bg-stone-900 hover:bg-stone-800 border border-stone-800 hover:border-stone-600 rounded-xl p-6 transition-all"
      >
        <div className="w-12 h-12 rounded-full bg-stone-800 group-hover:bg-red-950 flex items-center justify-center transition-colors flex-shrink-0">
          <Building2 className="w-5 h-5 text-stone-400 group-hover:text-[#dc2626] transition-colors" />
        </div>
        <div className="flex-1">
...
function ChapterMoney({ timeline }: {
  ...
  return (
    <section id="chapter-money" className="min-h-screen py-24 px-8 max-w-4xl mx-auto">
      ...
      {peakYear && (
        <div className="flex flex-wrap gap-4 mb-8">
          <div className="bg-stone-900 border border-stone-800 rounded-lg px-4 py-3">
            <p className="editorial-label text-stone-500 mb-1">Peak Year by Value</p>
            <p className="text-white font-bold">{peakYear.year} — {formatCompactMXN(peakYear.total_value)}</p>
          </div>
          {peakRiskYear && (
            <div className="bg-stone-900 border border-red-900/50 rounded-lg px-4 py-3">
      ...
      <div ref={ref} className="bg-stone-900 border border-stone-800 rounded-xl p-6 mb-6">
        <p className="editorial-label text-stone-500 mb-4">Contract Value by Year (MXN millions)</p>
        <AnimatePresence>
          {inView && (
            <motion.div
              initial={{ opacity: 0 }}
      ...
      {chartData.some((d) => d.risk > 0) && (
        <div className="bg-stone-900 border border-stone-800 rounded-xl p-6">
          <p className="editorial-label text-stone-500 mb-4">Average Risk Score by Year (%)</p>
          <div className="h-40">
            <ResponsiveContainer width="100%" height="100%">
    ...
    <section id="chapter-verdict" className="min-h-screen py-24 px-8 max-w-4xl mx-auto">
      ...
      <div className="bg-stone-900 border border-stone-800 rounded-2xl p-8 mb-8">
        <div className="flex items-start justify-between mb-6">
          <div>
            <p className="editorial-label text-stone-500 mb-2">Risk Indicator Score</p>
            ...
            <p className="text-stone-500 text-sm mt-1">out of 100</p>
          </div>
        ...
        {patternMeta && aria?.primary_pattern && (
          <div className="rounded-lg border p-4 mb-4" style={{ backgroundColor: patternMeta.bg, borderColor: patternMeta.color + '44' }}>
            ...
            <p className="text-stone-300 text-sm">{patternMeta.description}</p>
          </div>
        )}
        ...
        {aria?.memo_text && (
          <div className="border-l-2 border-[var(--color-accent)] pl-4 mt-4">
            <p className="editorial-label text-stone-500 mb-2">ARIA Intelligence Memo</p>
            <p className="text-stone-300 text-sm leading-relaxed whitespace-pre-line">{aria.memo_text}</p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 bg-stone-800 hover:bg-stone-700 text-white font-semibold rounded-xl px-5 py-3.5 transition-colors border border-stone-700"
        >
      ...
      {/* Disclaimer */}
      <div className="mt-10 p-4 bg-stone-950 border border-stone-800 rounded-xl">
        <div className="flex items-start gap-3">
          <FileText className="w-4 h-4 text-stone-600 flex-shrink-0 mt-0.5" />
          <p className="text-xs text-stone-600 leading-relaxed">
            <strong className="text-stone-500">Methodology note:</strong> Risk scores measure similarity to documented corruption patterns in 3.1M Mexican federal contracts (2002–2025). Scores are statistical indicators for investigation triage — not proof of wrongdoing. Model v6.0, vendor-stratified validation, AUC 0.861. See{' '}
            <Link to="/methodology" className="text-stone-500 underline hover:text-stone-300">
              full methodology
            </Link>.
          </p>
        </div>
  ...
  return (
    <div className="fixed right-5 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-3">
      {CHAPTERS.map((ch, idx) => (
        ...
        >
          <span className={cn(
            'text-xs opacity-0 group-hover:opacity-100 transition-opacity text-stone-400 hidden md:block',
  ...
  return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="text-center">
        ...
        <p className="text-stone-500 text-sm animate-pulse">Building investigation thread…</p>
      </div>
    </div>
  )
}
...
export default function RedThread() {
  ...
  if (!vendor) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        <p className="text-stone-400">Vendor not found.</p>
  ...
  return (
    <div className="relative min-h-screen bg-[var(--color-background)]" onScroll={handleScroll}>
      ...
      {/* Back nav */}
      <div className="sticky top-0 z-40 px-8 py-3 bg-[var(--color-background)]/80 backdrop-blur-sm border-b border-stone-900 flex items-center justify-between">
        <button
          onClick={() => navigate(-1)}
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-stone-600 uppercase tracking-widest">Red Thread</span>
          <span className="text-xs text-stone-700">·</span>
          <span className="text-xs text-stone-500 max-w-[200px] truncate">{vendor.name}</span>
        </div>
        <Link
          to={`/vendors/${id}`}
          className="text-xs text-stone-600 hover:text-stone-300 transition-colors flex items-center gap-1"
        >
