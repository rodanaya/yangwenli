...
function DayTooltip({ state }: { state: TooltipState }) {
  const { day, x, y } = state
  const d = new Date(day.date + 'T12:00:00Z')
  const formatted = d.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' })
  ...
  return (
    ...
    >
      <div className="bg-stone-900 border border-stone-700 rounded-md shadow-xl px-3 py-2.5 text-xs min-w-[190px]">
                ...
                {week.days.map((day, di) => {
                  ...
                  return (
                    <div
                      ...
                      role="gridcell"
                      aria-label={`${day.date}: ${day.total_contracts} contracts, ${(day.risk_rate * 100).toFixed(1)}% high-risk`}
                    />
                  )
                })}
              </div>
...
function computeStats(days: CalendarDay[]): YearStats {
  if (days.length === 0) {
    return { totalContracts: 0, highRiskContracts: 0, highRiskRate: 0, peakDay: null, decemberContracts: 0, januaryContracts: 0, decemberJanuaryRatio: null }
  }
  ...
  for (const day of days) {
    ...
    if (month === 11) decemberContracts += day.total_contracts
  ...
  return { totalContracts, highRiskContracts, highRiskRate, peakDay, decemberContracts, januaryContracts, decemberJanuaryRatio }
}
...
function Legend() {
  const swatches: { label: string; color: string }[] = [
    { label: 'No activity', color: 'hsl(220, 10%, 11%)' },
    { label: 'Low risk', color: getDayColor(100, 0.05, 3000) },
    { label: 'Medium risk', color: getDayColor(400, 0.15, 3000) },
    { label: 'High risk', color: getDayColor(800, 0.25, 3000) },
    { label: 'Critical risk', color: getDayColor(2000, 0.38, 3000) },
  ]

  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-stone-500">
      <span className="font-mono uppercase tracking-wider text-stone-600 text-[10px]">Risk level</span>
      {swatches.map((s) => (
      ...
      <div className="ml-4 flex items-center gap-1.5">
        ...
        <span>Activity (darker = more contracts)</span>
      </div>
    </div>
  )
}
...
export default function ProcurementCalendar() {
  ...
  // Find most common peak month (for annotation)
  const decemberAvgMonthly = useMemo(() => {
    if (days.length === 0) return null
    const monthly: Record<number, number> = {}
    for (const d of days) {
      const m = new Date(d.date + 'T12:00:00Z').getUTCMonth()
      monthly[m] = (monthly[m] ?? 0) + d.total_contracts
    }
  ...
  const decemberSpikeRatio = decemberAvgMonthly && decemberAvgMonthly > 0
    ? stats.decemberContracts / decemberAvgMonthly
    : null
    ...
    <div className="min-h-screen bg-stone-950 text-stone-200">
      ...
      <div className="border-b border-stone-800 px-6 py-5">
        <div className="max-w-6xl mx-auto">
          <EditorialHeadline
            section="Procurement Activity"
            headline="The Calendar of Suspicious Spending"
            subtitle="Every contract awarded in Mexico's federal government, day by day. Red indicates high-risk days. December glows red every year."
            className="mb-2"
          />
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <span className="text-[10px] font-mono uppercase tracking-widest text-stone-600 mr-2">Year</span>
          ...
          <span className="text-[9px] text-amber-600/70 font-mono ml-1">* election year</span>
        </div>
        ...
        >
          ...
          <HallazgoStat
            ...
            annotation={isLoading ? undefined : `${(stats.highRiskRate * 100).toFixed(1)}% of all contracts`}
          />
          <HallazgoStat
            value={
              isLoading || !stats.peakDay ? '—'
              : new Date(stats.peakDay.date + 'T12:00:00Z').toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' })
            }
            label="Peak day"
            color="border-amber-500"
            annotation={
              isLoading || !stats.peakDay ? undefined
              : `${formatNumber(stats.peakDay.total_contracts)} contracts`
            }
          />
          <HallazgoStat
            value={
            ...
            label="Dec vs Jan contracts"
            color="border-orange-500"
            annotation="December budget dump indicator"
          />
        </motion.div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            {isElectionYear && (
              <span className="text-xs text-amber-400 bg-amber-950/50 border border-amber-800 rounded px-2 py-0.5 font-mono flex items-center gap-1.5">
        ...
        {!isLoading && days.length > 0 && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="border border-orange-800/50 bg-orange-950/20 rounded-lg px-4 py-3 flex gap-3 items-start">
                  ...
                  <div>
                    <div className="text-sm font-semibold text-orange-300">
                      December {year}: {formatNumber(stats.decemberContracts)} contracts
                    </div>
                    <div className="text-xs text-stone-400 mt-0.5">
                      {decemberSpikeRatio.toFixed(1)}x the monthly average — year-end budget dumping detected
                    </div>
                  </div>
                </div>
              )}
              ...
              {isElectionYear && (
                <div className="border border-amber-800/50 bg-amber-950/20 rounded-lg px-4 py-3 flex gap-3 items-start">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-stone-400 mt-0.5">
                      Federal elections in {year} — procurement typically spikes
                      in pre-election months as incumbent governments accelerate spending
                    </div>
                  </div>
                </div>
              )}
              ...
              {stats.peakDay && stats.peakDay.risk_rate > 0.25 && (
                <div className={cn('border rounded-lg px-4 py-3 flex gap-3 items-start', getRiskBadgeColor(stats.peakDay.risk_rate))}>
                  <Info className="w-4 h-4 mt-0.5 shrink-0" />
                  <div>
                    <div className="text-sm font-semibold">
                      Highest-risk day:{' '}
                      {new Date(stats.peakDay.date + 'T12:00:00Z').toLocaleDateString('en-US', {
                        month: 'long', day: 'numeric', timeZone: 'UTC'
                      })}
                    </div>
                    <div className="text-xs text-stone-400 mt-0.5">
                      {(stats.peakDay.risk_rate * 100).toFixed(1)}% high-risk rate — {formatNumber(stats.peakDay.high_risk_contracts)} of {formatNumber(stats.peakDay.total_contracts)} contracts flagged
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="border-l-4 border-red-700 bg-red-950/20 rounded-r-lg px-5 py-4 mt-6">
          <div className="text-[10px] font-mono uppercase tracking-widest text-red-500 mb-2">Editorial Finding</div>
          <p className="text-stone-300 text-sm leading-relaxed">
            24 consecutive years of December spending spikes. Budget dumping is structural, not
            accidental. Every administration, regardless of party, exhausts remaining annual
            budget in December — creating a predictable window for rushed, lower-scrutiny
            contracting that accounts for a disproportionate share of high-risk procurement.
          </p>
        </div>
      </div>

      {/* Tooltip rendered at root level to avoid clipping */}
      {tooltip && <DayTooltip state={tooltip} />}
    </div>
  )
}
