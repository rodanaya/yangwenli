...
// Cell backgrounds derived from RISK_COLORS (Bible §3.10 — single palette).
// Low maps to the neutral muted background (never green).
const RISK_CELL_BG: Record<RiskLevel, string> = {
  critical: RISK_COLORS.critical, // #ef4444
  high:     RISK_COLORS.high,     // #f59e0b
  medium:   RISK_COLORS.medium,   // #a16207
  low:      'var(--color-text-muted)', // neutral, never green
}
...
export const SectorAdminHeatmap = memo(function SectorAdminHeatmap({
  ...
  // ── Rendered heatmap ──
  const kicker = lang === 'en' ? 'RUBLI · SECTOR × ADMINISTRATION' : 'RUBLI · SECTOR × ADMINISTRACIÓN'
  ...
  const source = lang === 'en'
    ? 'Source: RUBLI analysis · COMPRANET 2002–2025 · contract-weighted mean of avg_risk'
    : 'Fuente: análisis RUBLI · COMPRANET 2002–2025 · media ponderada por contratos de avg_risk'
  ...
  return (
    <>
      ...
      {ADMINISTRATIONS.map((admin) => {
        ...
        const bg = level ? RISK_CELL_BG[level] : 'var(--color-background-elevated)' // neutral no-data
