...
export function CommunityBubbles() {
  ...
  if (communities.length === 0) {
    return (
      <div className="flex items-center justify-center h-48 text-sm text-text-muted">
        {t('communityBubbles.empty', { defaultValue: lang === 'es' ? 'Sin datos de comunidades' : 'No community data available' })}
      </div>
    )
  }
  ...
  return (
    <EditorialChartFrame
      kicker={t('communityBubbles.kicker', { defaultValue: lang === 'es' ? 'RUBLI · Comunidades' : 'RUBLI · Communities' })}
      headline={t('communityBubbles.headline', { defaultValue: lang === 'es' ? 'Cómulos de co-licitantes' : 'Co-bidder clusters' })}
      subline={t('communityBubbles.subline', { defaultValue: lang === 'es' ? 'Tamaño = valor contratado · Color = nivel de riesgo' : 'Size = contract value · Color = risk level' })}
      footer={t('communityBubbles.footer', { defaultValue: lang === 'es' ? 'Fuente: COMPRANET · detección de comunidades de RUBLI' : 'Source: COMPRANET · RUBLI community detection' })}
    >
    <div className="relative">
          ...
          return (
            <g
              ...
              onMouseEnter={(e) => setTooltip({
                label: `${lang === 'es' ? 'Comunidad' : 'Community'} #${community.community_id}`,
              ...
              onMouseLeave={() => setTooltip(null)}
            >
              <circle cx={x} cy={y} r={r} fill={fill} opacity={opacity} stroke="var(--color-background-cream, var(--color-background-elevated))" strokeWidth={1} />
              {r > 22 && (
                <text
                  x={x} y={y}
                  textAnchor="middle" dominantBaseline="middle"
                  fontSize={r > 35 ? 9 : 7}
                  fill="var(--color-background-cream, var(--color-background-elevated))"
                  fontWeight="600"
      ...
      {tooltip && (
        ...
        >
          ...
          <p className="text-text-muted">
            {lang === 'es' ? 'Proveedores' : 'Vendors'}: <span className="text-text-primary">{tooltip.vendors}</span>
          </p>
          {tooltip.risk != null && (
            <p className="text-text-muted">
              {lang === 'es' ? 'Riesgo promedio' : 'Avg Risk'}: <span style={{ color: riskColor(tooltip.risk) }}>{tooltip.risk.toFixed(3)}</span>
            </p>
          )}
        </div>
      )}
      ...
      <div className="mt-2 flex items-center gap-3 flex-wrap text-[12px]">
        ...
        <span className="text-text-muted">
          {lang === 'es' ? 'Color = nivel de riesgo' : 'Color = risk level'}
        </span>
        {(['low','medium','high','critical'] as const).map(l => (
          <span key={l} className="flex items-center gap-1">
            ...
            {t(`communityBubbles.riskLevel.${l}`, { defaultValue: l })}
          </span>
        ))}
      </div>
    </div>
