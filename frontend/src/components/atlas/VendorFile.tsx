  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, height: variant === 'panel' ? '100%' : 'auto', background: variant === 'panel' ? 'var(--color-background-card)' : 'transparent' }}>
      ...
      {(() => {
        ...
        // Panel is docked to the (often short) chart frame, so the lower bands
        // scroll below the fold. On Windows the overlay scrollbar is invisible,
        // so the file READ as "cut off". Force an always-visible, non-obscuring
        // scrollbar (the honest affordance — the content can exceed the window,
        // so it must scroll) + breathing room so the last band never jams the CTA.
        return (
          <div className="rubli-file-scroll" style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
            <style>{`
              .rubli-file-scroll { scrollbar-width: thin; scrollbar-color: var(--color-text-muted) transparent; }
              .rubli-file-scroll::-webkit-scrollbar { width: 10px; }
              .rubli-file-scroll::-webkit-scrollbar-track { background: transparent; }
              .rubli-file-scroll::-webkit-scrollbar-thumb {
